...
<div class="pt-24 pb-20 max-w-md mx-auto px-3">
    ...
    <div class="flex justify-between items-center mb-5">
        <a href="{{ route('admin.belum_bayar') }}" class="text-gray-600 hover:text-gray-800">
...
// Tagihan perbulan = harga paket + biaya tambahan - diskon
$tagihanPerbulan = ($hargaPaket + $biayaTambahan1 + $biayaTambahan2) * (1 - $diskon/100);

// PPn 0% (sesuaikan kalau ada)
$ppn = 0;

// Total pembayaran
$totalBayar = $tagihanPerbulan + $ppn;

// Masa aktif (contoh, bisa ambil dari database)
$masaAktif = optional($pelanggan->tagihan()->latest('bulan')->first())->masa_aktif ?? '-';
...
$pesanWA = strtr($pesanTemplate, [
    ...
    '{bulan_tagihan}' => now()->translatedFormat('F Y'),
    '{jenis_paket}' => optional($pelanggan->package)->nama_paket ?? '-',
    '{paket}' => optional($pelanggan->package)->nama_paket ?? '-', // <== tambahan mapping paket
    ...
    '{status}' => $pelanggan->tagihan()->latest('bulan')->first()->status ?? '-',
...
<a href="https://wa.me/{{ $nomor }}?text={{ urlencode($pesanWA) }}"
   target="_blank"
   class="bg-green-500 text-white font-semibold py-3 rounded-lg w-full text-center block mb-4">
   KIRIM WA TAGIHAN
</a>
